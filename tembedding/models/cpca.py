...
class CPCA(object):
    ...
    to identify directions that have increased variance in the primary
    (foreground) dataset relative to a secondary (background) dataset.
    Then, those directions are used to project the data to a lower
    dimensional space.
    """
    def __init__(self, n_components=2, standardize=True, verbose=False):
        ...
        verbose: bool, default False
            wether to display debug messages.
        """
    ...
    def fit_transform(self, foreground, background, plot=False, gui=False,
                      alpha_selection='auto', n_alphas=40,  max_log_alpha=3,
                      n_alphas_to_return=4, active_labels = None, colors=None,
                      legend=None, alpha_value=None, return_alphas=False):
        """ Finds the covariance matrices of the foreground and background
        datasets, and then transforms the foreground dataset based on the
        principal contrastive components.

        Parameters: see self.fit() and self.transform() for parameter description
        """
        self.fit(foreground, background)
        return self.transform(
            dataset=self.fg, alpha_selection=alpha_selection,
            n_alphas=n_alphas, max_log_alpha=max_log_alpha,
            n_alphas_to_return=n_alphas_to_return, plot=plot, gui=gui,
            active_labels=active_labels, colors=colors, legend=legend,
            alpha_value=alpha_value, return_alphas=return_alphas)

    def fit(self, foreground, background, preprocess_with_pca_dim=None):
        """ Computes the covariance matrices of the foreground and background
        datasets.

        Parameters
        -----------
        foreground: array, shape (n_data_points, n_features)
            the dataset in which the interesting directions that we would
            like to discover are present or enriched.
        background: array, shape (n_data_points, n_features)
            the dataset in which the interesting directions that we would
            like to discover are absent or unenriched.
        preprocess_with_pca_dim: int, default None
            if this parameter is provided (and it is greater than n_features),
            then both the foreground and background datasets undergo a
            preliminary round of PCA to reduce their dimension to this
            number. If it is not provided but n_features > 10,000, a
            preliminary round of PCA is automatically performed to reduce
            the dimensionality to 1,000.
        """
        # Housekeeping
        ...
        if not(self.features_d == self.features_d_bg):
            raise ValueError("The dimensionality of the foreground and "
                             "background datasets must be the same.")

        # If the dimension is higher than preprocess_with_pca_dim, then PCA
        # is performed to reduce the dimensionality
        if preprocess_with_pca_dim is None:
            preprocess_with_pca_dim = 10000
        ...
        if (self.features_d > preprocess_with_pca_dim):
            ...
            if (self.verbose):
                var_ratio = int(100 * np.sum(pca.explained_variance_ratio_))
                print(f"Data dimensionality reduced to "
                      f"{str(preprocess_with_pca_dim)}. Percent variation "
                      f"retained: ~ {str(var_ratio)}%.")
        if (self.verbose):
            ...
            if (alpha_selection=='auto'):
                ...
                for j, fg in enumerate(transformed_data):
                    ...
                    for i, l in enumerate(np.sort(np.unique(self.active_labels))):
                        ...
                        plt.scatter(fg[idx,0],fg[idx,1], color=self.colors[i%len(self.colors)], alpha=0.6, label=str(l))
    ...
    def automated_cpca(self, dataset, n_alphas_to_return, n_alphas,
                       max_log_alpha):
        """
        This function performs contrastive PCA using the alpha technique on the
        active and background dataset. It automatically determines n_alphas
        important values of alpha up to based to the power of
        10^(max_log_alpha) on spectral clustering of the top subspaces
        identified by cPCA. The final return value is the data projected into
        the top (n_components) subspaces, which can be plotted outside of
        this function.
        """
    ...
    def all_cpca(self, dataset, n_alphas, max_log_alpha):
        """
        This function performs contrastive PCA using the alpha technique on the
        active and background dataset. It returns the cPCA-reduced data for
        all values of alpha specified, both the active and background, as
        well as the list of alphas.
        """
    ...
    def cpca_alpha(self, dataset, alpha=1):
        """
        Returns active and bg dataset projected in the cpca direction, as
        well as the top c_cpca eigenvalues indices. If specified, it returns
        the top_cpca directions.
        """
        ...
        reduced_dataset[:, 0] = (
            reduced_dataset[:, 0] * np.sign(reduced_dataset[0, 0]))
        reduced_dataset[:, 1] = (
            reduced_dataset[:, 1] * np.sign(reduced_dataset[0, 1]))
        return reduced_dataset

    def find_spectral_alphas(self, n_alphas, max_log_alpha, n_alphas_to_return):
        """
        This method performs spectral clustering on the affinity matrix of
        subspaces returned by contrastive pca, and returns (`=3) exemplar
        values of alpha.
        """
    ...
    def create_affinity_matrix(self, max_log_alpha, n_alphas):
        """
        This method creates the affinity matrix of subspaces returned by
        contrastive pca.
        """
        from math import pi
        alphas = np.concatenate(([0], np.logspace(-1, max_log_alpha, n_alphas)))
